...
def validate_config():
    ...
    required_envs = ["SECRET_KEY", "JWT_SECRET_KEY", "ADMIN_PASSWORD", "SENTRY_DSN"]
...
def set_security_headers(response):
    headers = {
        ...
        "Content-Security-Policy": "default-src 'self' 'unsafe-inline' data:; script-src 'self' https://unpkg.com; style-src 'self' https://unpkg.com;"
    }
...
# templates/track.html (updated with Leaflet map)
"""
<!DOCTYPE html>
<html>
<head>
    ...
    <script>
        ...
        function initMap() {
            map = L.map('map').setView([{{ shipment.origin_lat }}, {{ shipment.origin_lng }}], 5);
        ...
        function updateMap(data) {
            ...
            // Add checkpoint markers (blue) and polyline
            const checkpointCoords = [];
            data.checkpoints.forEach(cp => {
                checkpointCoords.push([cp.lat, cp.lng]);
                const marker = L.marker([cp.lat, cp.lng]).bindPopup(
                    `<b>${cp.label}</b><br>${cp.note || ''}<br>${cp.timestamp}`
                ).addTo(map);
                markers.push(marker);
            });

            if (checkpointCoords.length > 0) {
                polyline = L.polyline(checkpointCoords, { color: 'blue' }).addTo(map);
            }
